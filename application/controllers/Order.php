	...
	//Joe 2016/06/10
	private function deliver_details($order_code){
	...
	private function order_details($seller_id,$id)
	{
		...
		if ($row['total_price']&&$row['delivery_price'])
		{
			...
			$order_details['str_status'] =  $row['status'];
			foreach ($row as $k => $v)
			{
				$order_details[$k] = $v;
			}
	...
	private function refund_money($total_price, $delivery_price)
	{
	 ...
	 private function refund_to_buyer($buyer_id,$refund_money)
	 {
	 	$today = new DateTime();
